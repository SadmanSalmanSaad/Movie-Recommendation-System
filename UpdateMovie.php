...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
		...
		// Create a form for each delete button
                echo '<form method="post" action="process_update_movie.php">';
echo '<input type="hidden" name="movieID" value="' . $movieID . '">';
echo '<button type="submit" class="btn btn-danger btn-sm mt-2" style="background-color: #B80000; width: 100%;">Update Movie</button>';
echo '</form>';
